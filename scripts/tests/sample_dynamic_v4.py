...
if __name__ == "__main__":
    ...
    for i in trange(total_batch):
        ...
        c_idx = 1024 * torch.ones(batch_size, 1).long().cuda()
        c_idx_pos = 1026 * torch.ones(batch_size, 1).long().cuda()
        c_idx_seg = torch.zeros(batch_size, 1).long().cuda()

        idx_sample, idx_pos_sample = model.sample_from_scratch_with_fixed_fine_position(
            c_idx, c_idx_pos, c_idx_seg, 
        ...
        torchvision.utils.save_image(sampled_images, "{}/dynamic_v4_example_fixed_fine_{}.png".format(save_path_image, i))
        save_pickle(
        ...
        sampled_images = sampled_images * 0.5 + 0.5
        sampled_images = torch.clamp(sampled_images, 0, 1)
        torchvision.utils.save_image(sampled_images, "{}/dynamic_v4_example{}.png".format(save_path_image2, i))
        save_pickle(
                os.path.join(save_path_pickle2, 'samples_({}_{}).pkl'.format(i, total_batch)),
                sampled_images.cpu().numpy(),
            )