...
class BudgetConstraint_RatioMSE_DualGrain(nn.Module):
    def __init__(self, target_ratio=0., gamma=1.0, min_grain_size=8, max_grain_size=16):
        super().__init__()
    ...
    def forward(self, gate):
        ...
        loss_budget = self.loss(budget_ratio, target_ratio)

        return loss_budget
        
# class BudgetConstraint_RatioMSE_TripleGrain(nn.Module):
#     def __init__(self, target_ratio=0., gamma=1.0, min_grain_size=8, max_grain_size=32):
#         super().__init__()
#         self.target_ratio = target_ratio  # e.g., 0.8 means 80% are fine-grained
#         self.gamma = gamma
#         self.loss = nn.MSELoss()
...
#         beta = 1.0 * gate[:, 0, :, :] + 4.0 * gate[:, 1, :, :] + 16.0 * gate[:, 2, :, :]
#         beta = (beta.sum() / gate.size(0)) - self.const
#         budget_ratio = beta / self.max_const

#         target_radio = self.target_ratio * torch.ones_like(budget_ratio).to(gate.device)
#         loss_budget = self.loss(budget_ratio, target_radio)

#         return loss_budget

# class BudgetConstraint_SeperateRatioMSE_TripleGrain(nn.Module):
...
#         # 0 for coarse-grained, 1 for median-grained, 2 for fine grained
#         # gate: (batch, 3, min_grain_size, min_grain_size)
#         beta_median = 1.0 * gate[:, 0, :, :] + 4.0 * gate[:, 1, :, :]
...
#         beta_fine = 1.0 * gate[:, 0, :, :] + 16.0 * gate[:, 2, :, :]
...
#         loss_budget_fine = self.loss(budget_ratio_fine, target_ratio_fine)
...
if __name__ == "__main__":
    indices = torch.randint(0, 3, (10, 8, 8)).float()
    budget = BudgetConstraint_NormedSeperateRatioMSE_TripleGrain(
        target_fine_ratio=0., target_median_ratio=0., gamma=1.0, 
        min_grain_size=8, median_grain_size=16, max_grain_size=32
    )
    loss = budget(indices)
    print(loss)