...
class TripleGrainEncoder(pl.LightningModule):
    ...
    def forward(self, x, x_entropy):
        hw, hw0, hw1 = x.size(-1), x_entropy[0].size(-1), x_entropy[1].size(-1)
        x_entropy_expand_0 = x_entropy[0].unsqueeze(1).repeat_interleave(hw // hw0, -1).repeat_interleave(hw // hw0, -2)
        x_entropy_expand_1 = x_entropy[1].unsqueeze(1).repeat_interleave(hw // hw1, -1).repeat_interleave(hw // hw1, -2)
        
        x = torch.cat([x, x_entropy_expand_0, x_entropy_expand_1], dim=1)
