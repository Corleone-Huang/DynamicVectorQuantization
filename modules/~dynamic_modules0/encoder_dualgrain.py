...
class DualGrainRouter(nn.Module):
    ...
    #         if isinstance(module, nn.Linear):  # 0 for coarse-grained and 1 for fine-grained
    #             trunc_normal_(module.weight, std=.01)
    #             module.bias.data[0] = -bias_value
    #             module.bias.data[1] = bias_value
...
class DualGrainEncoder(pl.LightningModule):
    ...
    def __init__(self, 
        ...
        **ignore_kwargs
        ...
        self.router = DualGrainRouter(**router_config)
    ...
    def forward(self, x):
        ...
        gate = self.router(h_fine=h_fine, h_coarse=h_coarse)
        if self.training:
