...
class CrossAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        assert config.n_embd % config.n_head == 0
        assert config.cond_embd % config.n_head == 0
        # query, key, value projections for all heads
        self.query = nn.Linear(config.n_embd, config.n_embd)
        self.key = nn.Linear(config.cond_embd, config.n_embd)
        self.value = nn.Linear(config.cond_embd, config.n_embd)
        # regularization
        self.attn_drop = nn.Dropout(config.attn_pdrop)
        self.resid_drop = nn.Dropout(config.resid_pdrop)
        # output projection
    ...
    def forward(self, x, cond, mask = None):
        ...
        # calculate query, key, values for all heads in batch and move head forward to be the batch dim
        q = self.query(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        k = self.key(cond).view(B, Tc, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        v = self.value(cond).view(B, Tc, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
...
class Block(nn.Module):
    ...
    def forward(self, x, cond, mask = None, layer_past=None, return_present=False):
        ...
        cross_attn = self.cross_attn(self.ln2(x), cond, mask)
        x = x + cross_attn
...
class Transformer(nn.Module):
    def __init__(self, config, layer):
        ...
        for i in range(self.layer):
            self.model.append(Block(config))
    
    def forward(self, x, cond, mask = None):
        for i in range(self.layer):
            x = self.model[i](x, cond, mask)
...
class StackGPT(nn.Module):
    ...
    def __init__(self,
        ...
        config = StackGPTConfig(
            ...
            block_size = block_size, embd_pdrop = embd_pdrop, resid_pdrop = resid_pdrop, attn_pdrop = attn_pdrop, cond_embd = cond_embd,
        ...
        self.position_transformer = Transformer(config, config.position_layer)
        self.content_transformer = Transformer(config, config.content_layer)
    ...
    def forward(self, coarse_content, fine_content, coarse_position, fine_position, coarse_seg, fine_seg, content_target=None, coarse_position_target=None, fine_position_target=None, cond=None, cross_mask=None, **ignorekwargs):
        ...
        position_hidden = self.position_transformer(position_gpt_input, cond, cross_mask)
        ...
        content_hidden = self.content_transformer(content_gpt_input, cond, cross_mask)
    ...
    @torch.no_grad()
    def sample_coarse_position(self, coarse_content, coarse_position, coarse_seg, cond=None, cross_mask=None):
    ...
    @torch.no_grad()
    def sample_coarse_content(self, coarse_content=None, coarse_position=None, coarse_seg=None, position_hidden=None, cond=None, cross_mask=None):
    ...
    def sample_fine_position(self, coarse_content, fine_content, coarse_position, fine_position, coarse_seg, fine_seg, cond=None, cross_mask=None):
        content = torch.cat([coarse_content, fine_content], dim=1)
    ...
    def sample_fine_content(self, coarse_content, fine_content, coarse_position, fine_position, coarse_seg, fine_seg, position_hidden=None, cond=None, cross_mask=None):
        if position_hidden is None:
            content = torch.cat([coarse_content, fine_content], dim=1)
            content_embeddings = self.content_emb(content)
