...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("-l", "--logtype",type=str,default="tensorboard",nargs="?",help="log type",choices=["wandb","tensorboard"])
    # parser.add_argument("-d","--debug",type=str2bool,nargs="?",const=True,default=True,help="enable post-mortem debugging",)
...
if __name__ == "__main__":
    ...
    if opt.resume:  # resume from checkpoint
        ...
        if os.path.isfile(opt.resume):
            ...
            idx = len(paths)-paths[::-1].index("logs_debug")+1
        ...
        _tmp = logdir.split("/")
        nowname = _tmp[_tmp.index("logs_debug")+1]
    else:
        if opt.name:
            name = "_" + opt.name
        elif opt.base:
        ...
        if opt.postfix != "":
            ...
            nowname = now + name
        logdir = os.path.join("logs_debug", nowname)
