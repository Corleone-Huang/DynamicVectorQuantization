...
class TripleGrainVQModel(pl.LightningModule):
    def __init__(self,
        ...
        self.entropy_calculation_0 = Entropy(entropy_patch_size[0], image_size, image_size)
        self.entropy_calculation_1 = Entropy(entropy_patch_size[1], image_size, image_size)
    ...
    def encode(self, x):
        x_entropy_0 = self.entropy_calculation_0(x)
        x_entropy_1 = self.entropy_calculation_1(x)
        
        h_dict = self.encoder(x, [x_entropy_0,x_entropy_1])
        h = h_dict["h_triple"]
        ...
        return quant, emb_loss, info, grain_indices, gate, [x_entropy_0, x_entropy_1]
    ...
    def log_images(self, batch, **kwargs):
        ...
        indices_sum = grain_indices.size(0) * grain_indices.size(1) * grain_indices.size(2)
        fine_radio = (grain_indices==2).sum() / indices_sum
        median_radio = (grain_indices==1).sum() / indices_sum
        print("current ratios: fine: {}, median: {}".format(fine_radio, median_radio))
        ...
        x_entropy_0 = x_entropy[0].sub(x_entropy[0].min()).div(max(x_entropy[0].max() - x_entropy[0].min(), 1e-5))
        x_entropy_1 = x_entropy[1].sub(x_entropy[1].min()).div(max(x_entropy[1].max() - x_entropy[1].min(), 1e-5))
        log["entropy_map_0"] = draw_triple_grain_256res_color(images=x.clone(), indices=x_entropy_0, scaler=0.7)
        log["entropy_map_1"] = draw_triple_grain_256res_color(images=x.clone(), indices=x_entropy_1, scaler=0.7)
        return log
